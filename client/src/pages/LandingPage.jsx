        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="flex flex-col lg:flex-row items-center gap-16">
            ...
            <div className="lg:w-7/12 relative">
              <div className="relative rounded-xl shadow-2xl bg-slate-50 border border-slate-200 overflow-hidden">
                
                <div className="bg-white border-b border-slate-200 p-3 sm:p-4 flex justify-between items-center">
                   <div className="flex items-center gap-2">
                      <div className="w-7 h-7 sm:w-8 sm:h-8 rounded bg-blue-600 flex items-center justify-center text-white">
                        <LayoutDashboard size={16} />
                      </div>
                      <span className="font-bold text-slate-800 text-sm sm:text-base">BuildTrack</span>
                   </div>
                   <div className="flex items-center gap-2 sm:gap-4">
                      <Bell className="text-slate-500 w-4 h-4 sm:w-5 sm:h-5" />
                      <div className="flex items-center gap-2 text-right">
                        <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center font-bold text-xs">D</div>
                        <div className="hidden md:block">
                          <div className="font-bold text-slate-800 text-xs truncate max-w-[80px]">disha</div>
                ...
                <div className="flex">
                  <div className="w-48 lg:w-52 bg-white border-r border-slate-200 hidden md:block min-h-[500px] p-3">
                    <div className="space-y-1">
                      {[
                        ...
                        <div key={i} className={`flex items-center gap-2 px-2 py-2 rounded-lg text-xs font-medium truncate ${item.active ? 'bg-blue-50 text-blue-700' : 'text-slate-500 hover:bg-slate-50'}`}>
                          {item.icon}
                  ...
                  <div className="flex-1 p-4 sm:p-5 lg:p-6 bg-slate-50 font-sans overflow-hidden">
                    <div className="mb-4 sm:mb-5">
                      <h3 className="text-base sm:text-lg font-bold text-slate-800 truncate">Welcome back, disha!</h3>
                      <p className="text-xs sm:text-sm text-slate-500">Here's what's happening with your projects today.</p>
                    </div>

                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 mb-4 sm:mb-5">
                      <div className="bg-white p-2.5 sm:p-3 rounded-lg border border-slate-100 shadow-sm">
                        <div className="flex justify-between items-start mb-1.5">
                          <span className="text-[10px] sm:text-xs font-semibold text-slate-500 leading-tight">Total<br/>Projects</span>
                          <div className="p-1 rounded bg-blue-50 text-blue-600 flex-shrink-0"><FolderKanban size={12}/></div>
                        </div>
                        <div className="text-xl sm:text-2xl font-bold text-slate-800">1</div>
                        <div className="text-[10px] sm:text-xs text-slate-400 mt-0.5">1 active</div>
                      </div>

                      <div className="bg-white p-2.5 sm:p-3 rounded-lg border border-slate-100 shadow-sm">
                        <div className="flex justify-between items-start mb-1.5">
                          <span className="text-[10px] sm:text-xs font-semibold text-slate-500 leading-tight">Total<br/>Budget</span>
                          <div className="p-1 rounded bg-emerald-50 text-emerald-600 flex-shrink-0"><span className="font-bold text-xs">₹</span></div>
                        </div>
                        <div className="text-sm sm:text-xl lg:text-2xl font-bold text-slate-800 truncate">₹5L</div>
                        <div className="text-[10px] sm:text-xs text-slate-400 mt-0.5">70% utilized</div>
                      </div>

                      <div className="bg-white p-2.5 sm:p-3 rounded-lg border border-slate-100 shadow-sm">
                        <div className="flex justify-between items-start mb-1.5">
                          <span className="text-[10px] sm:text-xs font-semibold text-slate-500 leading-tight">Pending<br/>Milestones</span>
                          <div className="p-1 rounded bg-amber-50 text-amber-600 flex-shrink-0"><CheckSquare size={12}/></div>
                        </div>
                        <div className="text-xl sm:text-2xl font-bold text-slate-800">1</div>
                        <div className="text-[10px] sm:text-xs text-slate-400 mt-0.5 truncate">0 awaiting</div>
                      </div>

                       <div className="bg-white p-2.5 sm:p-3 rounded-lg border border-slate-100 shadow-sm">
                        <div className="flex justify-between items-start mb-1.5">
                          <span className="text-[10px] sm:text-xs font-semibold text-slate-500 leading-tight">Inventory<br/>Items</span>
                          <div className="p-1 rounded bg-red-50 text-red-600 flex-shrink-0"><AlertTriangle size={12}/></div>
                        </div>
                        <div className="text-xl sm:text-2xl font-bold text-slate-800">0</div>
                        <div className="text-[10px] sm:text-xs text-slate-400 mt-0.5 truncate">Items tracked</div>
                      </div>
                    </div>

                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4">
                      <div className="lg:col-span-2 bg-white p-3 sm:p-4 rounded-lg border border-slate-100 shadow-sm">
                        <h4 className="font-bold text-slate-800 mb-3 text-xs sm:text-sm">Budget Overview</h4>
                        <div className="flex items-end gap-4 sm:gap-6 h-32 sm:h-40 px-2 sm:px-4 border-l border-b border-slate-100 relative ml-8 sm:ml-10">
                          <div className="absolute -left-8 sm:-left-10 top-0 text-[9px] sm:text-[10px] text-slate-400 h-full flex flex-col justify-between py-0">
                             <span>₹6L</span>
                             <span>₹4L</span>
                             <span>₹3L</span>
                             <span>₹1L</span>
                          ...
                          <div className="w-1/3 bg-blue-300 h-[80%] relative rounded-t"></div>
                          <div className="w-1/3 bg-blue-600 h-[55%] relative rounded-t"></div>
                        </div>
                        <div className="text-center mt-2 text-[10px] sm:text-xs text-slate-500 truncate">spark project</div>
                        <div className="flex justify-center gap-3 sm:gap-6 mt-2 sm:mt-3 text-[10px] sm:text-xs font-medium">
                           <div className="flex items-center gap-1.5">
                              <div className="w-2.5 h-2.5 bg-blue-300 rounded-sm"></div> <span className="text-slate-600">Estimated</span>
                           </div>
                           <div className="flex items-center gap-1.5">
                              <div className="w-2.5 h-2.5 bg-blue-600 rounded-sm"></div> <span className="text-slate-600">Actual</span>
                           </div>
                        </div>
                      </div>

                      <div className="bg-white p-3 sm:p-4 rounded-lg border border-slate-100 shadow-sm">
                         <h4 className="font-bold text-slate-800 mb-3 text-xs sm:text-sm">Recent Activity</h4>
                         <div className="space-y-3">
                            <div className="flex gap-2 items-start">
                               <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-slate-100 flex items-center justify-center shrink-0">
                                  <FileBarChart size={12} className="text-slate-500"/>
                               </div>
                               <div className="min-w-0 flex-1">
                                  <div className="text-xs sm:text-sm font-semibold text-slate-800 truncate">spark project</div>
                                  <div className="text-[10px] sm:text-xs text-slate-400 truncate">Report by pooja - 11/12/2025</div>
                               </div>
                            </div>
